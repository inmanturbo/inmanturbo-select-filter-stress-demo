...
Route::get('/my-table', function () {
    ...
    $columns = [
        (new \App\ColumnData('year', 'Year'))
            ->class('text-xs p-1')
            ->headerClass('bg-gray-50 text-sm text-left whitespace-nowrap p-1')
        ...
        (new \App\ColumnData('debit', 'Debit'))
            ...
            ->headerClass('bg-gray-50 text-sm text-right')
            ->secondaryHeaderClass('text-xs text-right text-red-500')
            ->footerClass('text-xs text-right text-red-500')
            ->withTotal()
            ->toArray(),
